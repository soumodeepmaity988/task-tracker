...
const MIGRATIONS = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      await tryExec("ALTER TABLE contents ADD COLUMN video_urls TEXT NOT NULL DEFAULT '[]'");
      await tryExec("ALTER TABLE subtasks ADD COLUMN video_urls TEXT NOT NULL DEFAULT '[]'");
    },
  },
];
  ...
  return bRes.rows.map((b) => ({
    ...
    tasks: (tasksByB[b.id] || []).map((t) => ({
      ...
      subtasks: (subtasksByT[t.id] || []).map((st) => {
        const videoUrls = videoUrlsFromRow(st);
        return {
          id: st.id,
          title: st.title,
          notes: st.notes,
  ...
  for (const b of boards) {
    ...
    (b.tasks || []).forEach((t, i) => {
      ...
      (t.subtasks || []).forEach((st, j) => {
        const { videoUrl, videoUrls } = normalizeVideoUrls(st);
        stmts.push({
          sql: `INSERT INTO subtasks
            (id, task_id, title, notes, video_url, video_urls, doc_url, priority, status,
             source_content_id, position, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          args: [
            st.id, t.id, st.title, def(st.notes, ''),
            videoUrl, JSON.stringify(videoUrls),
            def(st.docUrl, ''),
  ...
  return sRes.rows.map((s) => ({
    ...
    topics: (topicsByS[s.id] || []).map((t) => ({
      ...
      contents: (contentsByT[t.id] || []).map((c) => {
        const videoUrls = videoUrlsFromRow(c);
        return {
          id: c.id,
          title: c.title,
          videoUrl: videoUrls[0] || '',
  ...
  for (const s of subjects) {
    ...
    (s.topics || []).forEach((t, i) => {
      ...
      (t.contents || []).forEach((c, j) => {
        const { videoUrl, videoUrls } = normalizeVideoUrls(c);
        stmts.push({
          sql: `INSERT INTO contents
            (id, topic_id, title, video_url, video_urls, doc_url, priority, status, notes, position, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          args: [
            c.id, t.id, c.title,
            videoUrl, JSON.stringify(videoUrls),
...
// Reads video_urls (new JSON array column) with fallback to legacy single video_url.
function videoUrlsFromRow(row) {
  const arr = safeParse(row.video_urls, null);
  if (Array.isArray(arr) && arr.length > 0) return arr.filter(Boolean);
  if (row.video_url) return [row.video_url];
  return [];
}
// Normalizes a JS object to a clean { videoUrl, videoUrls } pair for writes.
function normalizeVideoUrls(obj) {
  const raw = Array.isArray(obj?.videoUrls) ? obj.videoUrls : (obj?.videoUrl ? [obj.videoUrl] : []);
  const urls = raw.map(u => (u || '').trim()).filter(Boolean);
  return { videoUrl: urls[0] || '', videoUrls: urls };
}
